...
def base_dt(file_path, image_path, X_train, X_test, y_train, y_test, iteration):
    dt = DecisionTreeClassifier()
    dt.fit(X_train, y_train)

    if iteration == 0:
        plt.figure(figsize=(20, 10))
        if image_path == 'graphs/base_dt_abalone.png':
            plot_tree(dt, feature_names=X_train.columns, class_names=list(map(str, dt.classes_)), filled=True, rounded=True, max_depth=5)
...
def top_dt(file_path, image_path, X_train, X_test, y_train, y_test, iteration):
    top_dt_params = {
    ...
    if iteration == 0:
        ...
        if image_path == 'graphs/base_dt_abalone.png':
            plot_tree(best_dt, filled=True, feature_names=X_train.columns, class_names=True, max_depth=5)
        else:
